...
st.markdown(
    ...
    /* Style sidebar (if used elsewhere) */
...
if selected_tab == "🏠 Home":
    ...
    with st.expander("Add More Features"):
        for feature in feature_list:
            if feature not in top_7_features:
                label = feature
                is_required = False
                input_value = get_input(feature, label, is_required)
                input_features[feature] = input_value
...
# --- Model Performance Tab ---
    ...
    for _, row in evaluation_df.iterrows():
        ...
        **Overview:** {model_name} generally performs **{performance.lower()}** in predicting house prices based on the current metrics.
        """
